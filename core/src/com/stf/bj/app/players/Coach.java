...
public class Coach {
	...
	private String generateWagerMesssage(double wager) {
		return "Missed wager: true count was " + getTrueCount() + " and you waged " + wager + " instead of "
				+ getWager();
	}
	...
	private String generatePlayErrorString(Play idealPlay, Play playDone, Hand h, boolean sometimesCorrect) {
		...
		if (sometimesCorrect) {
			s += "\n         At some counts you would have made the correct play, but the true count was "
					+ getTrueCount();
		}
		return s;
	}
	...
	private String generateInsuranceMessage(boolean shouldHave) {
		return "Missed Insurance: True count was " + getTrueCount() + ", so you should " + (shouldHave ? "" : "NOT ")
